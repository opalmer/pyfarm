...
"""Provides the SortedDict example class.
"""
...
class SortedDict(object):
    ...
    def __repr__(self):
        ...
        >>> d = SortedDict(dict(s=1, a=2, n=3, i=4, t=5))
        >>> `d`
        "SortedDict({'a': 2, 'i': 4, 'n': 3, 's': 1, 't': 5})"
        >>> d = SortedDict({2: 'a', 3: 'm', 1: 'x'})
        >>> `d`
        "SortedDict({1: 'x', 2: 'a', 3: 'm'})"

        Alternative implementation using a list comprehension:

        return "SortedDict({%s})" % ", ".join(
               ["%r: %r" % (key, self.__dict[key]) \
                for key in self.__keys])
        """
        pieces = []
        for key in self.__keys:
            pieces.append("%r: %r" % (key, self.__dict[key]))
        return "SortedDict({%s})" % ", ".join(pieces)
