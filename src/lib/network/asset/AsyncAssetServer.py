...
#   os.chdir(options.root)
    ...
    try:
        settings = {"type" : 0,
                        "ip" : '',
                        "port" : 9030,
                        "root" : "C:\\Users\\opalmer\\Desktop\\test"
                        }

        req_handler = which[settings["type"]]
        s=Server(settings["ip"],settings["port"],req_handler)
        print req_handler.__name__, "running on port", settings["port"], "with root path", settings["root"]
        asyncore.loop()
    except KeyboardInterrupt:
        print "Ctrl+c pressed, killing server"
        sys.exit(0)
