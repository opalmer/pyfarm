...
Module also includes required network functions
    ...
    PyFarm is free SW: you can redistribute it and/or modify
    it under the terms of the GNU General Public License as published by
    the Free SW Foundation, either version 3 of the License, or
...
class QueSlaveServerThread(QThread):
    '''Wait for the master to ready the que, then receive the first CMD'''
    lock = QReadWriteLock()
    ...
    def run(self):
        ...
        while socket.state() == QAbstractSocket.ConnectedState:
            ...
            while True:
                ...
                print "Unpacking the CMD..."
                stream >> JOB >> FRAME >> SW >> CMD

                if JOB == 'TERMNATE_SELF':
                    socket.close()
                else:

                    print "%s %s" % (LOCAL_SOFTWARE[SW], CMD)
                    #print str(CMD)
                    #print SW
                    #print CMD
                    ACTION = QString("REQUESTING_WORK")
                    self.sendReply(socket, ACTION, JOB, SW, FRAME, CMD)
                    socket.waitForDisconnected()

                        # if the SW is not installed
#                        except KeyError:
#                            ACTION = QString("MISSING_SW")
#                            self.sendReply(socket, ACTION, JOB, FRAME, CMD)
#                            socket.waitForDisconnected()
    ...
    def sendReply(self, socket, ACTION, JOB, SW, FRAME, CMD):
        print "Requesting more work..."
        reply = QByteArray()
        stream = QDataStream(reply, QIODevice.WriteOnly)
        stream.setVersion(QDataStream.Qt_4_2)
        stream.writeUInt16(0)
        print ACTION, JOB, FRAME, SW, CMD
        stream << ACTION << JOB << FRAME << SW << CMD
        stream.device().seek(0)
        stream.writeUInt16(reply.size() - SIZEOF_UINT16)
        socket.write(reply)
...
class SendCmd(QTcpSocket):
    ...
    def __init__(self, client, port=QUE_PORT, parent=None):
        super(SendCmd, self).__init__(parent)
        self.client = client
        self.port = port
